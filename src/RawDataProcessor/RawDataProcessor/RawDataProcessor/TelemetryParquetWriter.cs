...
namespace RawDataProcessor
{
    internal class TelemetryParquetWriter
    {
        ...
        /// <returns>A collection of <see cref="ParquetContent"/> instances. Each instance
        /// contains an identifier and a Stream that contains the parquet-contents.</returns>
        internal IEnumerable<ParquetContent> CreateParquetContents(IEnumerable<TelemetryItem> items)
        {
            // First, group the metrics on DeviceId, since we're going to write files per
            // DeviceId and per Timestamp.
        ...
        private static Stream CreateParquetContentForGroup(IGrouping<int, TelemetryMessage> dayTelemetry)
        {
            ...
            {
                // Parquet is a column-based format, so it allows you to define a column and write all
                // contents for that column at once.
                // However, in this case our information is row-based.  Therefore, define a a table and
                // populate it with the appropriate contents and write the table at once.
                ...
                foreach (var telemetry in dayTelemetry)
                {
                    ...
                    // Populate each column for this row with the appropriate values. If we do not have a
